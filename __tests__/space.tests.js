...
describe('Person', () => {
  ...
  test('should dsiplay user years lived past on mercury', () => {
    ...
    expect(mercPast.showDisplay()).toEqual(` Your age on Mercury is : ${age}. You have lived ${exp * 1} years past the life expectancy on Mercury`)
  }); 
  test('should dsiplay user years left on mercury', () => {
    let mercLeft = new Person ("GlenBuck", 33)
    let age = mercLeft.mercuryAge(); 
    let exp = mercLeft.mercuryAge() - 75; 
    expect(mercLeft.showDisplay()).toEqual(` Your age on Mercury is : ${age}. You still have ${exp} years on Mercury`)
  }); 
  ...
  test('should dsiplay user years left on Venus', () => {
    let venLeft = new Person ("GlenBuck", 55)
    let age = venLeft.venusAge(); 
    let exp = venLeft.venusAge() - 75; 
    expect(venLeft.showDisplayVenus()).toEqual(` Your age on Venus is : ${age}. You still have ${exp * -1} years on Venus`)
  }); 
  ...
  test('should display user years lived past on Jupiter', () => {
    let jupLeft = new Person ("GlenBuck", 80)
